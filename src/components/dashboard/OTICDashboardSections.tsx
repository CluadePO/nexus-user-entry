...
// Mock data for service stages by company (simplified - no criticality)
const serviceStagesByCompany: Record<string, { total: number }[]> = {
  'c1': [{ total: 12 }, { total: 8 }, { total: 20 }, { total: 5 }],
  'c2': [{ total: 6 }, { total: 4 }, { total: 15 }, { total: 3 }],
  'c3': [{ total: 8 }, { total: 5 }, { total: 18 }, { total: 4 }],
  'c4': [{ total: 10 }, { total: 6 }, { total: 22 }, { total: 4 }],
  'c5': [{ total: 4 }, { total: 3 }, { total: 10 }, { total: 2 }],
  'c6': [{ total: 6 }, { total: 4 }, { total: 14 }, { total: 3 }],
  'c7': [{ total: 15 }, { total: 10 }, { total: 28 }, { total: 6 }],
  'c8': [{ total: 12 }, { total: 8 }, { total: 24 }, { total: 5 }],
  'c9': [{ total: 5 }, { total: 3 }, { total: 12 }, { total: 2 }],
};

// Mock data for filtered services by stage (all services combined, no criticality)
const stageServicesData: Record<string, any[]> = {
  'Por Emisión OC': [
...
export const ServiceStagesSection: React.FC = () => {
  ...
  const serviceStages = useMemo(() => {
    ...
    return serviceStagesBase.map((stage, stageIndex) => {
      const total = companyIds.reduce((acc, companyId) => {
        const companyData = serviceStagesByCompany[companyId]?.[stageIndex];
        if (companyData) {
          acc += companyData.total;
        }
        return acc;
      }, 0);
  ...
  const getFilteredServices = () => {
    ...
    return stageServicesData[selectedStage] || [];
  };
      ...
      <div className="flex items-stretch gap-1 mb-6">
        {serviceStages.map((stage, index) => (
          ...
          >
            ...
            <Tooltip title={`Clic para ver detalle de ${stage.name}`} placement="top">
              <div 
                className={`bg-gradient-to-b from-muted/50 to-muted/30 rounded-lg p-4 border transition-all h-full cursor-pointer ${
          ...
          <div className="bg-muted/30 rounded-lg p-4 border border-muted">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <h4 className="font-semibold">{selectedStage}</h4>
                <Tag color="blue">{filteredServices.length} servicios</Tag>
              </div>
              <Button 
                type="text" 
                size="small" 
            ...
            <Table 
              ...
              size="small"
              rowKey="id"
              locale={{ emptyText: 'No hay servicios en esta etapa' }}
            />
          </div>
        </div>
      )}
